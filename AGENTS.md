...
**Version:** `0.5.151` (`server/config.py` → `VERSION`) · **822** tests in `server/tests/run_tests.py`  
...
Keep badges at **0.5.151** / **822** until the suite or `VERSION` changes.  
Last **pushed** ship: *(pending)* (v0.5.151 combat).
**Docs map:** [docs/README.md](docs/README.md) — audience rules for both trees.  
Docs pass (**this run**): badges **0.5.151 / 822** · combat extract · human ≠ agent · no protocol dumps.
...
| `server/network/handlers/afk.py` | afk/away/busy/back (notices · census) |
| `server/network/handlers/safety.py` | quit · stuck/home (town recall · census) |
| `server/network/handlers/find.py` | find/search (filters · plain message · census) |
| `server/network/handlers/roll.py` | roll/dice (nearby icebreaker · sides validate) |
| `server/network/handlers/poke.py` | poke/nudge (private_social_delivery · near/far) |
| `server/network/handlers/thank.py` | thank/ty (private_social_delivery · near/far) |
...
| `server/network/handlers/combat.py` | attack/flee/battle spell (turn gate · AFK · defeat AOI · outcome announce · census) |
...
319. **`handlers/afk.py`:** afk/away/busy/back extracted from message_handler.
320. AFK ack includes **zone**, **online**, **afk_count**, **nearby_count**, **nearby_afk**, **in_combat**, plain **message** (zone/nearby suffix).
321. Nearby system notice on AFK flip unchanged (respect_ignore=False).
322. Tests: `test_features_v05132` + `test_mp_reliability_v05132`.
323. **`handlers/safety.py`:** quit/logout · stuck/unstuck/home extracted from message_handler.
324. Stuck already-home and teleport acks include **online**, **nearby_count**, **afk_count**.
325. Quit ack may include zone + online estimate; disconnect reason quit unchanged.
326. Tests: `test_features_v05133` + `test_mp_reliability_v05133`.
327. **`handlers/find.py`:** find/search extracted from message_handler.
328. Find always includes plain **message**, **nearby_count**, online/AFK/combat/zones census.
329. Filter tokens zone:/afk:/idle:/combat: multi-strip; social aliases; no coords on hits.
330. Tests: `test_features_v05134` + `test_mp_reliability_v05134`.
331. **`handlers/roll.py`:** roll/dice/d100 extracted from message_handler.
332. Sides validated before allow_chat (invalid never burns rate / clears AFK).
333. Roll chat includes plain **message**, zone, online, nearby_count on payload and roll card.
334. Tests: `test_features_v05135` + `test_mp_reliability_v05135`.
335. **`handlers/poke.py`:** poke/nudge/hey/attention/tap extracted from message_handler.
336. Poke uses **private_social_delivery** (refund_chat + restore_afk on fail).
337. Echo includes peer **nearby**/zone, plain message with peer_status_suffix, online/nearby_count.
338. Tests: `test_features_v05136` + `test_mp_reliability_v05136`.
339. **`handlers/thank.py`:** thank/thanks/ty/thx extracted from message_handler.
340. Thank uses **private_social_delivery** (refund_chat + restore_afk on fail).
341. Echo includes peer **nearby**/zone, plain message with peer_status_suffix, online/nearby_count.
342. Tests: `test_features_v05137` + `test_mp_reliability_v05137`.
343. **`handlers/askwhere.py`:** askwhere/locate extracted from message_handler.
344. Askwhere uses **private_social_delivery** (refund_chat + restore_afk on fail).
345. Echo includes peer **nearby**/zone; peer tip to /share @last unchanged.
346. Tests: `test_features_v05138` + `test_mp_reliability_v05138`.
347. **`handlers/share.py`:** share location extracted from message_handler.
348. Share uses **private_social_delivery**; note_share_to/from for soft reconnect.
349. Echo includes peer **nearby** + plain message with peer_status_suffix; recipient still gets zone+coords.
350. Tests: `test_features_v05139` + `test_mp_reliability_v05139`.
351. **`handlers/invite_cancel.py`:** cancel/uninvite extracted from message_handler.
...
359. **`handlers/invite_reply.py`:** accept/coming/invite_accept · decline/later/invite_decline/pass_invite extracted from message_handler.
360. Accept/decline uses **private_social_delivery** (refund_chat + restore_afk on fail; invite kept for retry).
361. Offline inviter clears soft-grace invite pointers (`invite_cleared`); coords only when AOI-near; echo census + peer_status_suffix.
362. Tests: `test_features_v05142` + `test_mp_reliability_v05142`.
363. **`handlers/emote.py`:** emote/emotes + shortcuts (wave/bow/cheer/…) extracted from message_handler.
364. Undirected AOI broadcast; directed far uses **private_social_delivery** (refund + restore AFK; no emote memory on fail).
365. Soft-grace note_emote_to/from; combat block; list catalog rate-exempt; self echo census + peer near/far.
366. Tests: `test_features_v05143` + `test_mp_reliability_v05143`.
367. **`handlers/whisper.py`:** whisper/tell/reply/r extracted from message_handler.
...
380. **`handlers/inventory.py`:** bag/inv · equip/wear · unequip/takeoff · discard/drop extracted from message_handler.
...
388. **`handlers/use_item.py`:** use/use_item/consume extracted from message_handler.
389. Combat herb only on awaiting_hero turn; AFK clear; wings publish_move AOI; defeat respawn path.
390. ITEM_USED + inventory echoes include online/nearby census; plain overworld messages.
391. Tests: `test_features_v05149` + `test_mp_reliability_v05149`.
...
| `tests.test_features_v05142` | accept/decline invite_reply WS · version |
| `tests.test_mp_reliability_v05142` | invite_reply extract · near coords · fail AFK · offline soft-grace |
| `tests.test_features_v05143` | emote/wave WS · list · version |
