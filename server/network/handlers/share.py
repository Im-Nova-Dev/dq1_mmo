"""Share location privately (extracted). Meetup helper — always opt-in.

Uses private_social_delivery so failed sends refund chat and restore AFK.
Recipient gets zone+coords; sharer echo includes peer near/far (no extra leak).
Tracks share_to / share_from for soft reconnect social memory.
"""
...
SHARE_TYPES = frozenset(
    {"share", "sharepos", "share_pos", "whereami_share", "here_i_am"}
)
ALL_TYPES = SHARE_TYPES
...
async def handle(
    ...
    """Dispatch share. Returns None if not a share message."""
    ...
    echo = dict(share_msg)
    echo["message"] = f"Location shared with {tname}{suffix}."
    if peer:
        if "nearby" in peer:
